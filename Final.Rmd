...
Emotion_Barplot(speech.war[3])
Emotion_Barplot(speech.war[6])
Emotion_Barplot(speech.war[8])
```
...
```
If I merge all speeches to their own group and draw an overall emotion bar plot, we can see a slight difference here: During both time, the dominant emotion of speeches was trust, but the ratio of trust was higher during war time. It's interesting that fear is in the top three emotions during peace time, while it's not a common emotion in war time speeches. I assume this happened because during that war time, presidents did not want to express too much negative emotion during the speech to make people feel worse.
